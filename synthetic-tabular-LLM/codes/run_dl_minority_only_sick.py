"""
🎯 Sick 数据集深度学习基线 - 只生成少数类样本（条件采样）
使用 CTGAN 和 TVAE 的条件采样功能，只生成 'sick' 类别的样本
"""
import pandas as pd
import numpy as np
import time
...
TARGET_COLUMN = 'Class'
MINORITY_CLASS = 'sick'  # 少数类标签
# ===========================================

print("=" * 70, flush=True)
print("🎯 Sick 数据集深度学习基线 - 只生成少数类样本（条件采样）", flush=True)
print("=" * 70, flush=True)
...
for cls, count in class_counts.items():
    print(f"      {cls}: {count} 条 ({count/len(y_train)*100:.2f}%)", flush=True)
...
print(f"\n   🎯 需要生成 {samples_needed} 条 '{MINORITY_CLASS}' 样本以平衡数据集", flush=True)
...
def evaluate_balanced_data(name, synthetic_minority_data):
    ...
    # 详细分类报告
    print(f"\n   📋 分类报告:")
    report = classification_report(y_test_enc, y_pred, target_names=le_y.classes_, digits=4)
    print(report)
...
print(f"   目标: 只生成 {samples_needed} 条 '{MINORITY_CLASS}' 样本", flush=True)
...
print(f"   正在使用条件采样生成 '{MINORITY_CLASS}' 类样本...", flush=True)
...
print(f"   ✅ 生成完成！")
print(f"   验证生成的样本类别:")
print(f"   {dict(syn_ctgan_minority[TARGET_COLUMN].value_counts())}")

# 保存
syn_ctgan_minority.to_csv(f"{SAVE_DIR}/Sick_CTGAN_minority_only.csv", index=False)
print(f"   ✅ 已保存: {SAVE_DIR}/Sick_CTGAN_minority_only.csv")
...
print(f"   ✅ 生成完成！")
print(f"   验证生成的样本类别:")
print(f"   {dict(syn_tvae_minority[TARGET_COLUMN].value_counts())}")

# 保存
syn_tvae_minority.to_csv(f"{SAVE_DIR}/Sick_TVAE_minority_only.csv", index=False)
print(f"   ✅ 已保存: {SAVE_DIR}/Sick_TVAE_minority_only.csv")
...
# ===========================================
print("\n" + "=" * 70)
print("🤖 [5/7] CTGAN 拒绝采样方法（生成后筛选）")
print("=" * 70)
print(f"   策略: 生成大量样本，然后只保留 '{MINORITY_CLASS}' 类")

# 生成更多样本以确保有足够的少数类
oversample_factor = 5  # 生成 5 倍的样本
total_samples = samples_needed * oversample_factor

print(f"   生成 {total_samples} 条样本，然后筛选出 {samples_needed} 条 '{MINORITY_CLASS}' 样本...")
...
print(f"\n   ✅ 拒绝采样完成！")
print(f"   原始生成: {len(syn_ctgan_all)} 条")
print(f"   其中 '{MINORITY_CLASS}': {(syn_ctgan_all[TARGET_COLUMN] == MINORITY_CLASS).sum()} 条")
print(f"   保留: {len(syn_ctgan_rejected)} 条")

# 保存
syn_ctgan_rejected.to_csv(f"{SAVE_DIR}/Sick_CTGAN_rejection_sampling.csv", index=False)
print(f"   ✅ 已保存: {SAVE_DIR}/Sick_CTGAN_rejection_sampling.csv")
...
print(f"\n   ✅ 拒绝采样完成！")
print(f"   原始生成: {len(syn_tvae_all)} 条")
print(f"   其中 '{MINORITY_CLASS}': {(syn_tvae_all[TARGET_COLUMN] == MINORITY_CLASS).sum()} 条")
print(f"   保留: {len(syn_tvae_rejected)} 条")

# 如果生成的少数类样本不够，继续生成
if len(syn_tvae_rejected) < samples_needed:
    print(f"   ⚠️ 警告：生成的 '{MINORITY_CLASS}' 样本不足，需要继续生成...")
    ...
    print(f"   ✅ 补充完成，最终保留: {len(syn_tvae_rejected)} 条")

# 保存
syn_tvae_rejected.to_csv(f"{SAVE_DIR}/Sick_TVAE_rejection_sampling.csv", index=False)
print(f"   ✅ 已保存: {SAVE_DIR}/Sick_TVAE_rejection_sampling.csv")
...
print(f"\n📊 最终结果对比:")
print(f"\n{'方法':<30} {'F1 Score':<12} {'Balanced Acc':<15}")
print("-" * 70)
print(f"{'CTGAN (条件采样)':<30} {f1_ctgan:<12.4f} {ba_ctgan:<15.4f}")
print(f"{'TVAE (条件采样)':<30} {f1_tvae:<12.4f} {ba_tvae:<15.4f}")
print(f"{'CTGAN (拒绝采样)':<30} {f1_ctgan_rej:<12.4f} {ba_ctgan_rej:<15.4f}")
print(f"{'TVAE (拒绝采样)':<30} {f1_tvae_rej:<12.4f} {ba_tvae_rej:<15.4f}")

print(f"\n📁 生成的文件:")
print(f"   - {SAVE_DIR}/Sick_CTGAN_minority_only.csv")
print(f"   - {SAVE_DIR}/Sick_TVAE_minority_only.csv")
print(f"   - {SAVE_DIR}/Sick_CTGAN_rejection_sampling.csv")
print(f"   - {SAVE_DIR}/Sick_TVAE_rejection_sampling.csv")

print("\n🎉 完成！所有方法都只生成了少数类样本来平衡数据集。")
print("=" * 70)
